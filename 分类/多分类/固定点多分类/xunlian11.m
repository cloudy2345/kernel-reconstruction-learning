%无回归项 各向异常性 logistic-loss
function moxing=xunlian11(x_train,y_train,x_node)   %返回分类模型
size1=size(x_train);
x_dim=size1(2);
n_train=size1(1);
size2=size(x_node);
...
while cha>10 | cha<-1
    ...
    loss1
    [theta0,loss2,exitflag2]=fmincon(@(theta) los1(theta,gamma0),theta0,[],[],[],[],zeros(x_dim,1),[]); %同性
    theta0'
    loss2
    cha=loss1-loss2;
end      %找到theta0与gamma
